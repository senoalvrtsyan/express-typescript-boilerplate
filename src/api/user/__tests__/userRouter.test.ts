...
describe('User API Endpoints', () => {
  describe('GET /users', () => {
    it('should return a list of users', async () => {
      ...
      expect(responseBody.responseData.length).toEqual(users.length);
      responseBody.responseData.forEach((user, index) => compareUsers(users[index] as User, user));
    });
  });
  ...
  describe('GET /users/:id', () => {
    it('should return a user for a valid ID', async () => {
      ...
      // Act
      const response = await request(server.app).get(`/users/${testId}`);
      const responseBody: ServiceResponse<User> = response.body;
      ...
      compareUsers(expectedUser, responseBody.responseData);
    });
    ...
    it('should return a not found error for non-existent ID', async () => {
      ...
      expect(responseBody.responseData).toBeNull();
    });

    it('should return a bad request for invalid ID format', async () => {
      // Act
      const invalidInput = 'abc';
      const response = await request(server.app).get(`/users/${invalidInput}`);
      const responseBody: ServiceResponse = response.body;
